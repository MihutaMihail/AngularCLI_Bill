...
let existingData = []; // Initialize existingData as an empty array

// Check if extractedData.json exists
if (fs.existsSync('extractedData.json')) {
  try {
    const jsonData = fs.readFileSync('extractedData.json', 'utf8');
    if (jsonData.trim() !== '') { // Check if jsonData is not empty
      existingData = JSON.parse(jsonData); // Parse JSON data
    }
  } catch (error) {
...
app.get('/extractedData/:id', (req, res) => {
  try {
    ...
    if (fs.existsSync('extractedData.json')) {
      const jsonData = fs.readFileSync('extractedData.json', 'utf8');
      if (jsonData.trim() !== '') { // Check if jsonData is not empty
        const extractedData = JSON.parse(jsonData); // Parse JSON data
...
let idCounter = 0; // Initialize a counter variable
