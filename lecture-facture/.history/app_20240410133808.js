...
app.put('/extractedData/:id/:field', (req, res) => {
  try {
    ...
    if (index !== -1) {
      // Check if the field to update exists in fieldValues
      if (existingData[index].fieldValues.hasOwnProperty(fieldToUpdate)) {
        ...
        res.status(400).json({ error: `Field '${fieldToUpdate}' does not exist` });
      }
    } else {
      res.status(404).json({ error: 'Data not found' });
    }
